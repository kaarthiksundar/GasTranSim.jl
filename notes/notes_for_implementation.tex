...
where $\mathcal{C} =  \dfrac{p_0}{\rho_0 v_0^2}$ and $\mathcal{M} = \dfrac{v_0}{a}$ are the Euler number and Mach number respectively.
...
The speed of sound $c$ in a gas  is given by $c^2 = \dfrac{d p}{d \rho}$. For an ideal gas, we get $c^2 = R_g T$, or in nondimensional terms  $\dfrac{d \bar{p}}{d \bar{\rho}} = 1/\mathcal{C} = \bar{c}^2 $.

For a simple CNGA equation, $p\cdot(b_1 + b_2 p) = \rho R_g T$, we can calculate the sound speed from $\dfrac{d \rho}{d p}$ since $\dfrac{d p}{d \rho} \cdot \dfrac{d \rho}{d p} = 1$. Thus  $c_{CNGA}^2 = \dfrac{R_g T}{b_1 + 2b_2 p}$. Since $b_1 > 1, b_2 > 0, p > 0$, $c_{CNGA} < c$.
...
Since we showed that $\bar{c} > \bar{c}_{CNGA}$, it is sufficient to use $\bar{c}$ in the CFL condition for non-ideal gases also. 
