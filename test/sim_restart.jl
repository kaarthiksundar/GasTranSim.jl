@testset "Simulation restart" begin
    ...
    ts_a = initialize_simulator(folder; case_name = "first_half", case_types = [:params])
    run_simulator!(ts_a)
    ...
    ts_b =
        initialize_simulator(folder; case_name = "second_half", case_types = [:params, :ic])
    ...
    pressure_node_5_cat = vcat(pressure_node_5_a, pressure_node_5_b[2:end])
    inflow_node_6_cat = vcat(inflow_node_6_a, inflow_node_6_b[2:end])
    outflow_node_2_cat = vcat(outflow_node_2_a, outflow_node_2_b[2:end])

    # average relative errors 
    e_pressure =
        sum(abs.(pressure_node_5_cat - pressure_node_5) ./ abs.(pressure_node_5)) /
        length(pressure_node_5)
    e_inflow =
        sum(abs.(inflow_node_6_cat - inflow_node_6) ./ abs.(inflow_node_6)) /
        length(inflow_node_6)
    e_outflow =
        sum(abs.(outflow_node_2_cat - outflow_node_2) ./ abs.(outflow_node_2)) /
        length(outflow_node_2)
