function build_ic(data::Dict{String,Any})::Dict{Symbol,Any}
    ...
    if haskey(data, "initial_compressor_flow") && haskey(data, "compressor_flow")
        (throw(
            ICException("both keys compressor_flow  and initial_compressor_flow present"),
        ))
    end

    compressor_flow_key = ""
    (haskey(data, "initial_compressor_flow")) &&
        (compressor_flow_key = "initial_compressor_flow")
