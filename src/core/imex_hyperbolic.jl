...
function _check_CFL(ts::TransientSimulator, x::Vector{Float64}, n::Int64, pipe_id::Int64)
    v = maximum( abs.( x[n+1 : 2*n] ./ x[1 : n] ) )
    v_max = 0.1 / nominal_values(ts, :mach_num)
    if v > v_max
        throw(CFLException(string("$pipe_id. velocity $v exceeds $v_max")))
    end
    return
end
...
function _solve_pipe_state_imex_hyperbolic!(
    ...
    convection_fluxes = _precompute_convection_fluxes_imex(ts, pipe_id, rho_old, phi_old)
...
function _precompute_convection_fluxes_imex(
    ts::TransientSimulator,
    pipe_id::Int64,
    rho_old::AbstractVector{T},
    phi_old::AbstractVector{T})::Matrix{T} where {T<:Real}
    n = length(rho_old)
    conv = zeros(T, 2, n + 1)  # interface i_left in 0:n maps to col i_left+1
    ...
    rho_to_bc = T(get_density(ts, ref(ts, :node, to_node)["pressure"]))
    phi_fr = T(ref(ts, :pipe, pipe_id)["fr_mass_flux"])
    phi_to_bc = T(ref(ts, :pipe, pipe_id)["to_mass_flux"])

    numerical_convection_flux_inplace!(view(conv, :, 1), ts, rho_fr, phi_fr, rho_old[1], phi_old[1])
    for i_left = 1:(n - 1)
        numerical_convection_flux_inplace!(view(conv, :, i_left + 1), ts, rho_old[i_left], phi_old[i_left], rho_old[i_left + 1], phi_old[i_left + 1])
    end
    numerical_convection_flux_inplace!(view(conv, :, n + 1), ts, rho_old[n], phi_old[n], rho_to_bc, phi_to_bc)
...
function _pipe_residual_imex_hyperbolic!(
    ...
    convection_fluxes::AbstractMatrix{T},
    rho_from::T,
    ...
    flux_plus = zeros(T, 2)
    flux_minus = zeros(T, 2)


    for i = 1 : n
        r1 = (rho[i] - rho_old[i])
        r2 = (phi[i] - phi_old[i]) - dt * imex_source_phi(rho[i], phi[i], drag_coeff, g_coeff)
        if i == 1
            imex_numerical_flux!(flux_plus, ts, rho[i], phi[i], rho[i+1], phi[i+1], p_coeff)
            imex_numerical_flux!(flux_minus, ts, rho_from, phi_from, rho[i], phi[i], p_coeff)
        elseif i == n
            imex_numerical_flux!(flux_plus, ts, rho[i], phi[i], rho_to, phi_to, p_coeff)
            imex_numerical_flux!(flux_minus, ts, rho[i-1], phi[i-1], rho[i], phi[i], p_coeff)
        else
            imex_numerical_flux!(flux_plus, ts, rho[i], phi[i], rho[i+1], phi[i+1], p_coeff)
            imex_numerical_flux!(flux_minus, ts, rho[i-1], phi[i-1], rho[i], phi[i], p_coeff)
        end
        r1 += mu * ((flux_plus[1] - flux_minus[1]) + (convection_fluxes[1, i + 1] - convection_fluxes[1, i]))
        r2 += mu * ((flux_plus[2] - flux_minus[2]) + (convection_fluxes[2, i + 1] - convection_fluxes[2, i]))
        r[i] += r1
        r[i + n] += r2
    end
...
function imex_source_phi(rho::T, phi::T, drag_coeff::Real, grav_coeff::Real)::T where {T<:Real}
    d = T(drag_coeff)
    g = T(grav_coeff)
    return -d * phi * abs(phi) / rho + g * rho
end
...
function imex_numerical_flux!(
    ...
    rho_right::T,
    phi_right::T,
    pressure_coeff::Real) where {T<:Real}
    pcoeff = T(pressure_coeff)
    cL = sqrt(get_pressure_prime(ts, rho_left))
    cR = sqrt(get_pressure_prime(ts, rho_right))
    s = smooth_max(cL, cR, T(1e-6))
    f1_left = phi_left
    f2_left = pcoeff * get_pressure(ts, rho_left)
    f1_right = phi_right
    f2_right = pcoeff * get_pressure(ts, rho_right)
    out[1] = (f1_right + f1_left - s * (rho_right - rho_left)) / T(2)
    out[2] = (f2_right + f2_left - s * (phi_right - phi_left)) / T(2)
    return out
end
...
function convection_rusanov_speed(ts::TransientSimulator, U_left, U_right)
    T = promote_type(typeof(U_left[1]), typeof(U_left[2]), typeof(U_right[1]), typeof(U_right[2]))
    epsilon = T(1e-6)
...
function numerical_convection_flux_inplace!(
    out::AbstractVector{T},
    ts::TransientSimulator,
    rho_left::T,
    phi_left::T,
    rho_right::T,
    phi_right::T) where {T<:Real}
    s = convection_rusanov_speed(ts, (rho_left, phi_left), (rho_right, phi_right))
    f2_left = phi_left^2 / rho_left
    f2_right = phi_right^2 / rho_right
    out[1] = -s * (rho_right - rho_left) / T(2)
    out[2] = (f2_right + f2_left - s * (phi_right - phi_left)) / T(2)
    return out
end
...
function _pipe_jacobian_imex_hyperbolic!(
    ...
    for i = 1 : n
        ...
        u_mat = zeros(T, 2, 2)
        l_mat = zeros(T, 2, 2)
        jsrc21 = drag_coeff * (phi[i] * abs(phi[i]) / (rho[i]^2)) + g_coeff
        jsrc22 = -drag_coeff * (T(2) * abs(phi[i]) / rho[i])
        d_mat[1, 1] = I11
        d_mat[1, 2] = zero(T)
        d_mat[2, 1] = -dt * jsrc21
        d_mat[2, 2] = I22 - dt * jsrc22
        if i == 1
            U_right = [ rho[i+1], phi[i+1] ]
            U_left = [rho_from, phi_from]
            J_i_plus_half_left, J_i_plus_half_right = imex_jacobian_numerical_flux(ts, [rho[i], phi[i]], U_right, p_coeff)
            J_i_minus_half_left, J_i_minus_half_right = imex_jacobian_numerical_flux(ts, U_left, [rho[i], phi[i]], p_coeff)
