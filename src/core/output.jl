...
function update_output_data!(ts::TransientSimulator, state::OutputState, data::OutputData)
    ...
    for (i, _) in ref(ts, :node)
        data.node[i]["pressure"] =
            Spline1D(state.time_pressure, state.node[i]["pressure"], k = 1)
        data.final_state["nodal_pressure"][i] = ref(ts, :node, i, "pressure")
    end

    if params(ts, :load_adjust) == true
        for (i, _) in ref(ts, :node)
            data.node[i]["load_reduction"] =
                Spline1D(state.time_flux, state.node[i]["load_reduction"], k = 1)
        end
    end

    for (i, pipe) in ref(ts, :pipe)
        data.pipe[i]["fr_mass_flux"] =
            Spline1D(state.time_flux, state.pipe[i]["fr_mass_flux"], k = 1)
        data.pipe[i]["to_mass_flux"] =
            Spline1D(state.time_flux, state.pipe[i]["to_mass_flux"], k = 1)
    ...
    for i in keys(get(ref(ts), :compressor, []))
        data.compressor[i]["flow"] =
            Spline1D(state.time_flux, state.compressor[i]["flow"], k = 1)
