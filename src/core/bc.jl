function build_bc(data::Dict{String,Any})::Dict{Symbol,Any}
    ...
    for (i, value) in get(data, "boundary_pslack", [])
        ...
        val = value["value"]
        bc[:node][id] =
            Dict("spl" => Spline1D(t, val, k = 1), "control_type" => pressure_control)
    end
    ...
    for (i, value) in get(data, "boundary_nonslack_flow", [])
        ...
        val = value["value"]
        bc[:node][id] =
            Dict("spl" => Spline1D(t, val, k = 1), "control_type" => flow_control)
    end
    ...
    for (i, value) in get(data, "boundary_compressor", [])
        ...
        bc[:compressor][id] = Dict("spl" => CompressorControl(t, bc_type, val))
    end
