...
Base.showerror(io::IO, e::CFLException) =
    print(io, "CFL condition fails, time-step too large for component ", e.var, "!")

Base.showerror(io::IO, e::NetworkException) =
    print(io, "Network topology incompatible with simulator; ", e.var, "!")

Base.showerror(io::IO, e::ICException) =
    print(io, "Initial condition missing: ", e.var, "!")
...
initial_nodal_pressure(ts::TransientSimulator, id::Int64) = ts.initial_conditions[:node][id]
...
function get_compressor_control(
    ts::TransientSimulator,
    id::Int64,
    t::Real,
)::Tuple{CONTROL_TYPE,Float64}
    ...
    if CONTROL_TYPE(control_type) == flow_control
        control_type_prev, val =
            ts.boundary_conditions[:compressor][id]["spl"](t - ts.params[:dt]/2)
...
function evaluate(sp::CompressorControl, t::Real)::Tuple{Any,Float64}
    ...
    value = sp.value
    (isapprox(t, time[1], rtol = TOL)) ||
        (t <= time[1]) && (return control_type[1], value[1])
    (isapprox(t, time[end], rtol = TOL)) ||
