...
# animation for the 1-pipe fast transients using ideal EoS
...
first_file = output_json * "1-pipe-fast-0.json"
...
for i = 1:60
    file = output_json * "1-pipe-fast-$i.json"
...
slack_pressure = Observable(
    Point2f[(0.0, pascal_to_psi(first_data["initial_pipe_pressure"]["1"]["value"][1]))],
)
...
record(
    fig,
    output_anim * "1-pipe-fast-pipe-animation.mp4",
    timestamps;
    framerate = framerate,
) do t
    time[] = t
