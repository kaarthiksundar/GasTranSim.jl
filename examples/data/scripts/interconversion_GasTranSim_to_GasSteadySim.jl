...
"""
function create_datafile_for_steady_state_time_marching(
    folder_in::AbstractString,
    folder_out::AbstractString;
    network_filename::AbstractString = "network.json",
    bc_steady_filename::AbstractString = "bc.json",
    params_steady_filename::AbstractString = "params.json",
    t_initial::Float64 = 0.0,
    t_ramp::Float64 = 21600.0,
    t_final::Float64 = 86400.0,
    p_init::Float64 = 1.0e6,
    flow_init::Float64 = 0.0,
    cmpr_init::Float64 = 1.0,
)
    ...
    params_dict["simulation_params"] =
        get(params_steady_data, "params", get(params_steady_data, "simulation_params", ""))
    ...
    params_dict["simulation_params"]["Courant number (must be between 0 and 1, recommended value is 0.9"] =
        0.95
...
"""
function create_datafile_for_steady_state_simulation_from_transient_datafile(
    ...
    for (key, compressor_dict) in bc_data["boundary_compressor"]
        ...
        bc_steady["boundary_compressor"][key]["control_type"] =
            compressor_dict["control_type"][1]
    ...
    params_dict = Dict{String,Any}("simulation_params" => Dict())
    params_dict["simulation_params"] = Dict(
    ...
    if haskey(params_data["simulation_params"], "nominal_pressure")
        params_dict["simulation_params"]["nominal_pressure"] =
            params_data["simulation_params"]["nominal_pressure"]
    end
    if haskey(params_data["simulation_params"], "nominal_velocity")
        params_dict["simulation_params"]["nominal_velocity"] =
            params_data["simulation_params"]["nominal_velocity"]
    end
    if haskey(params_data["simulation_params"], "nominal_length")
        params_dict["simulation_params"]["nominal_length"] =
            params_data["simulation_params"]["nominal_length"]
    end
    if haskey(params_data["simulation_params"], "nominal_density")
        params_dict["simulation_params"]["nominal_density"] =
            params_data["simulation_params"]["nominal_density"]
    end
