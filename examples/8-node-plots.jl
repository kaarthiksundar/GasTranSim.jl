...
f = Figure(backgroundcolor = RGBf(0.98, 0.98, 0.98), size = (700, 700))
ga = f[1, 1] = GridLayout()
gb = f[2, 1] = GridLayout()
gc = f[3, 1] = GridLayout()
gd = f[4, 1] = GridLayout()
...
ax3 = Axis(
    ...
    xticks = [0, 6, 12, 18, 24],
    yticks = [120, 150, 180],
    title = rich("Withdrawal at node (kgs", superscript("-1"), ")"),
)
ax4 = Axis(
    gd[1, 1],
    xticks = [0, 6, 12, 18, 24],
...
coarse =
    lines!(ax2, t, round.(pressure_node_5/1e6; digits = 2), alpha = 0.7, color = :orange)
fine = lines!(
    ax2,
    t,
    round.(pressure_node_5_fine/1e6; digits = 2),
    alpha = 0.7,
    linestyle = (:dashdot, :dense),
    color = :green,
)
